...
- 번호식 **계층 메뉴**로 명령을 선택해 서버로 전송한다. 각 메뉴에서 **`0`은 뒤로/종료**, 그냥 **Enter**는 메뉴 재출력.
- **Ctrl+C(SIGINT)** 로만 정상 종료(`QUIT` 전송 후 소켓 정리). 그 외 시그널은 무시 → 강제 종료 방지.

#### 메뉴 구조 (어떤 번호를 누르면 어떤 명령이 가는지)

```
====== 원격 장치 제어 ======          ← 메인 메뉴
 1) LED ─────────────┐
 2) 부저(Buzzer) ─────┤
 3) 조도센서(CDS) ────┤  번호 입력 → 서브메뉴 진입
 4) 7세그먼트(FND) ───┘
 5) 상태(STATUS)       → STATUS 즉시 전송
 0) 종료(QUIT)         → QUIT 전송 후 종료
============================
...
2) 부저(Buzzer)
   1) 멜로디 켜기(ON) → BUZZER ON
   2) 끄기(OFF)       → BUZZER OFF
   0) 뒤로

3) 조도센서(CDS, PCF8591)
   1) 자동연동 켜기   → CDS ON  (모니터링 화면 진입, Enter로 CDS OFF 후 복귀)
   2) 현재값 읽기     → CDS READ
   3) 임계값 설정     → 0~255 입력 → CDS THRESHOLD <값>
   0) 뒤로

4) 7세그먼트(FND)
   1) 카운트다운 시작 → 0~9 입력 → FND <n>
   2) 정지(STOP)      → FND STOP
   0) 뒤로
```

- **예시**: LED를 밝게 켜려면 → 메인 `1`(LED) → `3`(밝게). 7세그 5초 카운트다운 → 메인 `4` → `1` → `5`.
- **CDS 자동연동**: `3 → 1`로 켜면 매초 `EVT CDS`를 표시하는 모니터링 화면으로 들어가고,
